...
[release_notes]
# v89.0.1 - Release Banner & AI-Aware TODOs
description = Surfaced the release version directly in the UI header and auto-capture log warnings/errors into TODOs with ready-to-run LLM prompts.
changes = Navigation now renders the synced release chip beside PokerTool branding while master logging writes deduplicated TODO entries that embed guidance prompts for AI-assisted triage, keeping operational fixes actionable.
status = stable
