...
- start.py will:
    ...
    * If code_scan.py is missing or its CLI is unknown, run an internal syntax scanner (py_compile).
...
class _L:
    ...
    @classmethod
    def info(cls, msg: str, **kw: Any) -> None: _log.info(cls._fmt(msg, **kw))
    @classmethod
    def warning(cls, msg: str, **kw: Any) -> None: _log.warning(cls._fmt(msg, **kw))
    @classmethod
    def error(cls, msg: str, **kw: Any) -> None: _log.error(cls._fmt(msg, **kw))
    @classmethod
    def critical(cls, msg: str, **kw: Any) -> None: _log.critical(cls._fmt(msg, **kw))
    @classmethod
    def debug(cls, msg: str, **kw: Any) -> None: _log.debug(cls._fmt(msg, **kw))
...
def setup_global_exception_handler() -> None:
    def _hook(exc_type, exc, tb):
        logger.critical("Global exception", exception=exc)
...
def initialize_logging() -> None:
    ...
    logger.info("APPLICATION STARTUP", argv=sys.argv, cwd=os.getcwd(), python_path=list(sys.path), environment_vars=safe_env)
...
def _detect_scanner_flags(python: str, script: Path) -> dict[str, bool]:
    ...
    try:
        res = subprocess.run([python, str(script), "--help"], cwd=str(ROOT), capture_output=True, text=True)
...
def run_code_scan(python: str, scan_path: Path) -> int:
    """
    Try to run ./code_scan.py. Auto-detect flags. If not present or failing, fallback to internal scan.
    Env overrides:
    ...
    if completed.returncode != 0:
        logger.warning("code_scan.py non-zero or incompatible; running internal syntax scan", returncode=completed.returncode)
        return _internal_quick_syntax_scan(scan_path)
    return 0
...
def write_file(path: Path, content: str, mode_exec: bool = False) -> None:
...
def main():
    ...
    print("  2) Optional envs: START_NO_SCAN=1, START_SCAN_DRY_RUN=1, START_NO_BACKUP=1, START_SCAN_PATH=/path")
    print("  3) To pass args to poker_go.py, use:  ./start.py -- --your --poker-go-args")
