...
# project: pokertool
# file: tools/hotfix_pokertool.py
# version: v28.0.0
...
"""
Hotfix script for pokertool GUI issues.
Patches files to handle missing dependencies gracefully.
"""
...
def patch_poker_gui():
    """Patch poker_gui.py to handle import errors gracefully."""
    p = ROOT / 'poker_gui.py'
    if not p.exists():
        return 'missing'
...
def replace_poker_init():
    """Replace poker_init.py with a minimal version."""
...
def compile_skip_backups():
    """Compile Python files while skipping backup directories."""
...
def main():
    """Main hotfix entry point."""
    print('[hotfix] poker_gui.py: ', patch_poker_gui())
    print('[hotfix] poker_init.py: ', replace_poker_init())
