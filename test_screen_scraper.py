...
class TestPokerScreenScraper:
    ...
    def setup_method(self):
        ...
        # Flop
                                flop = [Card('A', Suit.SPADE), Card('K', Suit.HEART), Card('Q',
                                    Suit.DIAMOND)]
        ...
        # Mock seat activity detection
                                    with patch.object(self.scraper, '_is_seat_active',
                                        return_value = True):
        ...
        # Setup mocks
                                                        mock_capture.return_value = np.ones((768,
                                                            1024, 3), dtype = np.uint8)
                                                        mock_pot.return_value = 100.0
                                                        mock_hero.return_value = [Card('A',
                                                            Suit.SPADE), Card('K', Suit.HEART)]
                                                        mock_board.return_value = []

                                                        seat1 = SeatInfo(1, is_active = True,
                                                            stack_size = 1000.0)
                                                        seat2 = SeatInfo(2, is_active = True,
                                                            stack_size = 1500.0)
