...
class TestCompletePokerWorkflows:
    ...
    def test_complete_cash_game_session(self):
        ...
        # Game parameters
        positions = [Position.UTG, Position.MP1, Position.CO, Position.BTN, Position.SB,
            Position.BB]
        stack_bb = 100
        ...
        # All final analyses should be valid
                                for result in session_results:
                                    assert result['final_decision'] in ['FOLD', 'CALL', 'RAISE',
                                        'CHECK']
                                    ...
                                    def test_tournament_progression_simulation(self):
                                        ...
                                        tournament_stages = [
                                        {'name': 'Early', 'stack_bb': 100, 'blinds': (0.5, 1.0),
                                            'players': 9}, 
                                        {'name': 'Middle', 'stack_bb': 50, 'blinds': (1.0, 2.0),
                                            'players': 7}, 
                                        {'name': 'Bubble', 'stack_bb': 15, 'blinds': (2.0, 4.0),
                                            'players': 5}, 
        ...
        # Simulate 3 tables with different dynamics
                                                        tables = [
                                                        {'id': 1, 'avg_stack': 80, 'players': 9,
                                                            'style': 'tight'}, 
                                                        {'id': 2, 'avg_stack': 60, 'players': 8,
                                                            'style': 'aggressive'}, 
                                                        {'id': 3, 'avg_stack': 120, 'players': 7,
                                                            'style': 'loose'}, 
                                                        ]

                                                        for table in tables:
            # Test multiple positions at each table
                                                            for position in [Position.UTG,
                                                                Position.CO, Position.BTN]:
                                                                hole = [Card('A', Suit.SPADE),
                                                                    Card('K',
                                                                    Suit.HEART)]  # Strong hand

                                                                analysis = analyse_hand(
                                                                hole = hole, board=[],
                                                                    position = position, 
                                                                stack_bb = table['avg_stack'],
                                                                    pot = 3.0, to_call = 2.0, 
