                ...
                try:
                    ...
                    if attempt < max_retries:
                        log.warning(
                            ...
                            attempt + 1, max_retries + 1, func.__name__, e, current_delay
                        )
                        time.sleep(current_delay)
                        current_delay *= backoff
                    else:
                        log.error("All %d attempts failed for %s", max_retries + 1, func.__name__)
    ...
    def call(self, func: Callable, *args, **kwargs) -> Any:
        ...
        try:
            ...
            if self.failure_count >= self.failure_threshold:
                self.state = 'OPEN'
                log.error("Circuit breaker opened due to %d failures", self.failure_count)
