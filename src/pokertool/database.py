...
class ProductionDatabase:
    ...
    def _create_postgresql_schema(self):
        ...
        BEGIN
            ...
            SELECT 
                ...
                0::NUMERIC as avg_pot_size,  -- Would extract from metadata
    ...
    def create_session(self, notes: Optional[str] = None, metadata: Optional[Dict] = None) -> str:
        ...
        with db_guard('creating session'):
            if self.config.db_type == DatabaseType.POSTGRESQL:
                with self.get_connection() as conn:
                    with conn.cursor() as cursor:
                        cursor.execute(
                            "INSERT INTO game_sessions (session_id, notes, metadata) VALUES (%s, %s, %s)",
                            (session_id, notes, metadata_json)
                        )
                        conn.commit()
            else:
                    ...
                    with conn.cursor(cursor_factory=RealDictCursor) as cursor:
                        ...
                        return {
                            ...
                            'pool_stats': {
                                ...
                                'current_connections': len(self.connection_pool._pool) if hasattr(self.connection_pool, '_pool') else 0
                            }
                        }
            else:
                # SQLite stats
    ...
    def migrate_from_sqlite(self, sqlite_path: str) -> int:
        ...
        try:
            ...
            cursor = sqlite_conn.execute("""
                SELECT hand_text, board_text, analysis_result, timestamp, user_hash, session_id
                FROM poker_hands
                ORDER BY timestamp
            """)
            ...
            with self.get_connection() as pg_conn:
                with pg_conn.cursor() as pg_cursor:
                    for row in cursor:
                        pg_cursor.execute("""
                            INSERT INTO poker_hands 
                            (hand_text, board_text, analysis_result, timestamp, user_hash, session_id)
                            VALUES (%s, %s, %s, %s, %s, %s)
                        """, tuple(row))
                        migrated_count += 1
            ...
            # Migrate sessions
            cursor = sqlite_conn.execute("""
                SELECT session_id, start_time, end_time, hands_played, notes
                FROM game_sessions
            """)
                ...
                with pg_conn.cursor() as pg_cursor:
                    for row in cursor:
                        pg_cursor.execute("""
                            ...
                            (session_id, start_time, end_time, hands_played, notes)
                            VALUES (%s, %s, %s, %s, %s)
