...
class TableDiagramWindow(tk.Toplevel):
    ...
    def __init__(self, parent=None):
        logger.debug('Creating instance of TableDiagramWindow', 
    ...
    def _draw_table(self):
        ...
        # Stage text
        self.canvas.create_text(center_x, 30, text=self.state.stage, 
    ...
    def _draw_dealer_button(self, canvas_w: int, canvas_h: int):
        ...
        # Calculate offset based on seat position
        offset_x = 40
        offset_y = 0
        ...
        self.canvas.create_text(button_x, button_y, text='D', 
    ...
    def _draw_blinds(self, canvas_w: int, canvas_h: int):
        ...
        # Find dealer position in active seats
    ...
    def _draw_blind_chip(self, seat: int, text: str, color: str, canvas_w: int, canvas_h: int):
        ...
        self.canvas.create_oval(chip_x - radius, chip_y - radius, 
                               chip_x + radius, chip_y + radius, 
                               fill=color, outline='black', width=1)
        self.canvas.create_text(chip_x, chip_y, text=text, 
                               font=('Arial', 9, 'bold'), fill='white')

    def _draw_pot_area(self, center_x: int, center_y: int):
        """Draw the pot area in the center of the table."""
        self.canvas.create_oval(center_x - 60, center_y - 30, 
    ...
    @log_exceptions
    def update_state(self, active_players: Set[int], hero_seat: int, dealer_seat: int,
                     pot: float, to_call: float, stage: str, equity: Optional[float] = None):
        """Update the table state and redraw."""
        logger.debug('Entering TableDiagramWindow.update_state',
                     active_players=active_players, hero_seat=hero_seat, dealer_seat=dealer_seat)
