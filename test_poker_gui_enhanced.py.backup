...
# applied_improvements: ['gui_optimization_tests', 'visual_components_tests']
# summary: Comprehensive unit tests for enhanced poker GUI
# ---
...
class TestTableVisualization(unittest.TestCase):
    ...
    @patch('tkinter.Canvas')
    def test_dealer_button_drawing(self, mock_canvas):
        ...
        table.players = {
            1: PlayerInfo(seat=1, is_active=True, stack=100, bet=0, is_dealer=True)
        }
        
        # Draw dealer button
        table._draw_dealer_button(800, 500)
    ...
    @patch('tkinter.Canvas')
    def test_pot_drawing(self, mock_canvas):
        """Test pot drawing."""
        from poker_gui_enhanced import TableVisualization
    ...
    @patch('tkinter.Canvas')
    def test_board_cards_drawing(self, mock_canvas):
        ...
        # Draw board
        table._draw_board(400, 250)
        
        # Should create 3 cards
        self.assertEqual(table.create_rectangle.call_count, 3)
        self.assertEqual(table.create_text.call_count, 3)
...
class TestEnhancedPokerAssistant(unittest.TestCase):
    ...
    @patch('tkinter.Tk')
    def test_position_calculation(self, mock_tk):
        """Test position calculation based on dealer."""
        from poker_gui_enhanced import EnhancedPokerAssistant
        
        with patch('poker_gui_enhanced.MODULES_LOADED', True):
            app = EnhancedPokerAssistant()
            app.dealer_var = Mock()
            ...
            self.assertEqual(position, Position.BUTTON)
            
            # Test small blind
            app.dealer_var.get = Mock(return_value=9)
            app.players[9].is_active = True
            position = app._calculate_position()
            self.assertEqual(position, Position.SMALL_BLIND)
    
    @patch('tkinter.Tk')
    def test_blinds_update(self, mock_tk):
    ...
    def test_hand_analysis(self, mock_analyse, mock_tk):
        ...
        with patch('poker_gui_enhanced.MODULES_LOADED', True):
            ...
            # Set hole cards
            app.hole_cards = [('A', Suit.SPADES), ('K', Suit.HEARTS)]
    ...
    @patch('tkinter.Tk')
    def test_clear_cards(self, mock_tk):
        """Test clearing selected cards."""
        from poker_gui_enhanced import EnhancedPokerAssistant
        ...
        # Set some cards
        app.hole_cards = [('A', Suit.SPADES), ('K', Suit.HEARTS)]
        app.board_cards = [('Q', Suit.DIAMONDS)]
        ...
        self.assertEqual(len(app.hole_cards), 0)
        self.assertEqual(len(app.board_cards), 0)
...
class TestGUIIntegration(unittest.TestCase):
    ...
    @patch('tkinter.Tk')
    def test_player_toggle_updates_table(self, mock_tk):
    ...
    @patch('tkinter.Tk')
    def test_dealer_change_updates_blinds(self, mock_tk):
