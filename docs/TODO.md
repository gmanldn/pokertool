...
- [x] [P0][M] Implement OpenAI provider — ✅ Complete: GPT-4 Turbo with function calling and structured output `ai_providers/openai_provider.py`
...
- [x] [P0][M] Connect LLM provider (OpenAI/Anthropic) — ✅ Complete: Added LLM integration in `langchain_memory_service.py` with support for both OpenAI GPT-4 and Anthropic Claude. Auto-detects available providers via OPENAI_API_KEY and ANTHROPIC_API_KEY environment variables. Enhanced chat_about_poker() with full LLM integration, system prompts for poker expertise, conversation history context, and fallback response when LLM unavailable. File: `src/pokertool/langchain_memory_service.py:35-46,379-500`
...
- [x] [P2][S] Cross-browser testing — ✅ Complete: Created comprehensive Playwright cross-browser test suite (`tests/cross-browser/browser-compatibility.spec.ts`, 18 tests) validating Chrome, Firefox, Safari (WebKit), and Edge in light/dark modes. Tests cover: homepage loading, navigation, dark mode toggle, responsive design, CSS Grid/Flexbox, modern JS features (ES6+, async/await, fetch), localStorage/sessionStorage, WebSocket, Material-UI rendering, console errors, and basic accessibility. Updated `playwright.config.ts` with 8 browser configurations. Added 5 npm scripts: `test:cross-browser`, `test:cross-browser:chromium/firefox/webkit/edge`. Documentation: `docs/testing/CROSS_BROWSER_TESTING.md` with troubleshooting, CI/CD integration, browser version matrix, and debugging guide.

## Now (P0: highest ROI)

- [ ] [P0][S] Fix frontend compilation failed — Frontend compilation blocked. Error: Failed to compile.. Detected on 2025-10-23. Check logs/frontend_compile_errors.log for full details. Application auto-shutdown due to blocking error. Total occurrences: 1.
- [ ] [P0][S] Fix frontend unknown error — Frontend compilation blocked. Error: ERROR in src/components/BankrollManager.tsx:382:36. Detected on 2025-10-23. Check logs/frontend_compile_errors.log for full details. Application auto-shutdown due to blocking error. Total occurrences: 1.
- [ ] [P0][S] Fix frontend unknown error — Frontend compilation blocked. Error: ERROR in src/components/BankrollManager.tsx:382:36. Detected on 2025-10-23. Check logs/frontend_compile_errors.log for full details. Application auto-shutdown due to blocking error. Total occurrences: 1.
- [ ] [P0][S] Fix frontend unknown error — Frontend compilation blocked. Error: ERROR in src/components/BankrollManager.tsx:382:36. Detected on 2025-10-23. Check logs/frontend_compile_errors.log for full details. Application auto-shutdown due to blocking error. Total occurrences: 1.
- [ ] [P0][S] Fix frontend unknown error — Frontend compilation blocked. Error: ERROR in src/components/BankrollManager.tsx:382:36. Detected on 2025-10-23. Check logs/frontend_compile_errors.log for full details. Application auto-shutdown due to blocking error. Total occurrences: 1.
- [ ] [P0][S] Fix frontend unknown error — Frontend compilation blocked. Error: ERROR in src/components/BankrollManager.tsx:382:36. Detected on 2025-10-23. Check logs/frontend_compile_errors.log for full details. Application auto-shutdown due to blocking error. Total occurrences: 1.
- [ ] [P0][S] Fix frontend unknown error — Frontend compilation blocked. Error: ERROR in src/components/BankrollManager.tsx:382:36. Detected on 2025-10-23. Check logs/frontend_compile_errors.log for full details. Application auto-shutdown due to blocking error. Total occurrences: 1.
- [x] [P0][S] Fix frontend compilation failed — ✅ Complete: Frontend now builds successfully with only minor ESLint warnings (unused variables). Build size: 267KB gzipped main bundle.
- [x] [P0][S] Fix frontend unknown error — ✅ Complete: Frontend builds successfully. Fixed ESLint warnings in BackendStatus.tsx, EmptyState.tsx, TableView.tsx, ActionRecommendationCard.tsx, EquityChart.tsx, and SmartHelper.tsx by removing unused imports (HourglassEmptyIcon, FilterAltOff, Snackbar, LineChart, Line, Divider, Science), removing unused function (getStatusColor), fixing unnecessary useMemo dependency (dealerPosition), and adding explicit comments for ESLint rules. Build completes with no errors or warnings.
...
- [x] ✅ [P1][M] Create pot odds calculator integration — Calculate and display pot odds in real-time based on detected pot/bet. `src/pokertool/core.py:pot_odds`
...
- [x] [P1][M] Implement detection result validation — ✅ Complete: DetectionSanityChecks module validates logical consistency with SanityCheckSeverity levels (WARNING/ERROR/CRITICAL), comprehensive validation rules, error collection and reporting, automatic rejection of impossible states. DetectionAccuracyMetrics tracks accuracy over time with confidence distribution tracking. Files: `src/pokertool/detection_sanity_checks.py` (433 lines), `src/pokertool/detection_accuracy_tracker.py` (209 lines).
...
- [x] [P0][M] Add comprehensive detection events for all state changes — ✅ Complete: Created DetectionStateDispatcher (470 lines) providing comprehensive state-change tracking and event emission. Features: TableState dataclass tracking pot (size, side pots), board cards, hero cards, player states (stacks, names, positions, active status), game state (street, hand_id, button), performance (FPS, frame time). DetectionStateDispatcher class with intelligent change detection (emits only on actual changes), automatic street transition detection (preflop→flop→turn→river), hand lifecycle management (start/end events), frame correlation IDs for event chains, event counting and statistics. Methods: update_pot(), update_board_cards(), update_hero_cards(), update_player(), emit_player_action(), update_performance(), reset_hand(), begin_frame()/end_frame(). All methods use structured event emission functions from detection_events.py. Global singleton via get_dispatcher(). Comprehensive test suite (14 tests, all passing) covering pot changes, board updates, hero cards, player updates, actions, performance, hand management, frame correlation, stats. Ready for integration into poker_screen_scraper_betfair.py. Files: `src/pokertool/detection_state_dispatcher.py`, `tests/test_detection_state_dispatcher.py`.
- [x] [P0][S] Implement event batching for high-frequency updates — ✅ Complete: DetectionEventBatcher batches events every 100ms (configurable) with max 50 events per batch, thread-safe queue, automatic flushing on size limit, background worker thread, and callback-based emission. File: `src/pokertool/detection_event_batcher.py` (51 lines).
- [x] [P0][M] Add structured event payloads with full context — ✅ Complete: Enhanced detection_events.py with comprehensive structured event system. Enhanced emit_detection_event() to accept event_id and correlation_id for event tracking and correlation. Added 7 specialized event emission functions: emit_pot_event() (pot size, changes, side pots, confidence), emit_card_event() (cards with rank/suit, card type, confidence), emit_player_event() (seat, name, stack, stack changes, position, confidence), emit_action_event() (seat, action type, amount, player, confidence), emit_state_change_event() (state transitions with reason), emit_performance_event() (FPS, frame time, memory, CPU). All functions use structured data classes (PotEventData, CardEventData, PlayerEventData, ActionEventData, StateChangeEventData, PerformanceEventData) with automatic confidence level calculation (high ≥0.9, medium ≥0.7, low <0.7), automatic message formatting, and validation via DetectionEventSchema. Added emit_structured_event() for custom events. All events include full context: timestamp, confidence, confidence_level, detection_method, duration_ms, type-specific data. Events auto-convert to dictionaries for WebSocket transmission. Ready for comprehensive detection event emission across all detection modules. Files: `src/pokertool/detection_events.py:210-533`.
- [x] [P0][S] Create event type enum and schema validation — ✅ Complete: DetectionEventType enum with 16 event types (pot, card, player, action, board, button, blind, state_change, hand_start/end, street_change, system, error, warning, info, performance, fps, latency). EventSeverity enum for levels. DetectionEventData base class with timestamp/confidence/method/duration. Specialized data classes: PotEventData, CardEventData, PlayerEventData, etc. Full validation and to_dict() support. File: `src/pokertool/detection_events.py` (614 lines).
- [ ] [P1][M] Implement event replay system — Record all events to disk. Allow replay for debugging. `src/pokertool/event_recorder.py`
- [x] ✅ [P1][S] Add event filtering by type/severity — Allow clients to subscribe to specific event types. `src/pokertool/detection_websocket.py:subscribe`
...
- [x] [P1][M] Implement detection error tracking — ✅ Complete: log_detection_error() function logs errors with full exception info, traceback, detection type, context dict, structured logging with extra fields. File: `src/pokertool/detection_error_logger.py` (20 lines).
...
- [x] ✅ [P0][S] Implement detection bottleneck identification — ✅ Complete: BottleneckIdentifier class analyzes CPU and execution time metrics to identify performance bottlenecks. Detects high CPU operations (>50%), slow operations (>100ms), and top time consumers. Generates optimization recommendations with priority ranking. Supports OCR/template matching specific suggestions. Global singleton via get_bottleneck_identifier(). Comprehensive test coverage (12 tests). Files: `src/pokertool/performance_telemetry.py:788-1167`, `tests/test_bottleneck_identifier.py` (232 lines).
...
- [x] ✅ [P1][S] Implement CPU usage tracking per detection type — ✅ Complete: DetectionCPUTracker class tracks CPU usage and execution time per detection type (OCR, template matching, color detection). Context manager API via `with cpu_tracker.track('type')`. Tracks CPU%, min/max CPU%, execution time, call count. Thread-safe with sliding window (default: 100 samples). Global singleton via get_cpu_tracker(). Summary statistics across all types. Comprehensive test coverage (22 tests). Files: `src/pokertool/performance_telemetry.py:550-911`, `tests/test_cpu_tracker.py` (375 lines).
...
- [x] [P0][S] Implement session statistics queries — ✅ Complete: Created SessionStatisticsCalculator (330 lines) for comprehensive poker session analysis. Features: SessionStats dataclass tracking total hands, wins/losses, VPIP (Voluntarily Put $ In Pot %), PFR (Pre-Flop Raise %), 3-bet %, aggression factor, win rate, bet sizing averages, position-based stats, confidence scores, time tracking, hands per hour. Calculated properties: win_rate, vpip, pfr, three_bet_pct, aggression_factor, net_profit, hands_per_hour. SessionStatisticsCalculator with methods: calculate_session_stats() (by session_id, time range, or last N hands), calculate_multiple_sessions(), calculate_position_stats(), get_recent_session_stats(). Automatically computes position-specific win rates, average bet ratios, average pot sizes, session duration. Supports flexible querying by session ID, time range, or recent hands. Ready for API integration and dashboard display. Files: `src/pokertool/session_statistics.py`.
- [x] [P0][M] Create opponent profiling queries — ✅ Complete: Created comprehensive OpponentProfiler (370 lines) for player tendency analysis. Features: PlayerStyle enum (TAG/LAG/TAP/LAP/Rock/Maniac/Fish/Unknown), OpponentProfile dataclass tracking VPIP, PFR, 3-bet %, fold to 3-bet %, c-bet %, aggression frequency, avg bet sizing, position-specific VPIP/PFR, win rate, net profit, style classification (tight/loose/aggressive/passive). Auto-classification based on thresholds: Rock (VPIP<15%), TAG (VPIP<25%, PFR>15%), LAG (VPIP>30%, PFR>20%), Fish (VPIP>45%, PFR<10%), Maniac (VPIP>50%, PFR>40%). OpponentProfiler methods: get_opponent_profile() (min hands filter), get_all_opponent_profiles(), compare_opponents(), get_opponents_by_style(), get_toughest_opponents() (TAG/LAG with high win rate), get_weakest_opponents() (Fish/LAP/TAP with low win rate). Position-specific tendencies by seat. Ready for HUD display and strategy adjustment. Files: `src/pokertool/opponent_profiling.py`.
