...
# project: pokertool
# file: backups/develop_20250922_053042/backups/develop_20250922_053042/update-v24.py
# version: v20.0.0
...
def main():
    ...
    commit_message = f"Auto-commit v25 update - {timestamp}"
    ...
    # Check if v25 tag already exists and handle appropriately
    print("Checking for existing v25 tag...")
    stdout, stderr = run_git_command("git tag -l v25", cwd=repo_path)
    
    if stdout == "v25":
        print("⚠️  v25 tag already exists. Creating v25-update tag instead...")
        tag_name = f"v25-update-{timestamp}"
        run_git_command(f'git tag -a {tag_name} -m "Version 25 update - {timestamp}"', cwd=repo_path)
    else:
        print("Creating v25 tag...")
        tag_name = "v25"
        run_git_command(f'git tag -a {tag_name} -m "Version 25 release"', cwd=repo_path)
