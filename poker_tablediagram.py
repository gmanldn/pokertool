...
class TableDiagramWindow(tk.Toplevel):
    ...
    def _draw_player(self, seat: int, canvas_w: int, canvas_h: int):
        ...
        radius = 25
        self.canvas.create_oval(x - radius - 2, y - radius - 2, x + radius +
            2, y + radius + 2, fill=border_color, outline='', width=0)
        self.canvas.create_oval(x - radius, y - radius, x + radius, y +
            radius, fill=bg_color, outline='', width=0)
        self.canvas.create_text(x, y - 5, text=f'P{seat}', font=('Arial',
            14, 'bold'), fill=text_color)
        if is_hero:
            self.canvas.create_text(x, y + 10, text='YOU', font=('Arial', 8,
    ...
    def _draw_dealer_button(self, canvas_w: int, canvas_h: int):
        ...
        button_y = y + offset_y
        radius = 15
        self.canvas.create_oval(button_x - radius, button_y - radius,
            button_x + radius, button_y + radius, fill=C_DEALER, outline=
            'black', width=2)
        self.canvas.create_text(button_x, button_y, text='D', font=('Arial',
            12, 'bold'), fill='black')
    ...
    def _draw_blind_chip(self, seat: int, text: str, color: str, canvas_w:
        int, canvas_h: int):
        ...
        chip_y = y + offset_y
        radius = 12
        self.canvas.create_oval(chip_x - radius, chip_y - radius, chip_x +
            radius, chip_y + radius, fill=color, outline='black', width=1)
        self.canvas.create_text(chip_x, chip_y, text=text, font=('Arial', 9,
            'bold'), fill='white')

    def _draw_pot_area(self, center_x: int, center_y: int):
        """Draw the pot area in the center of the table."""
        self.canvas.create_oval(center_x - 60, center_y - 30, center_x + 60,
            center_y + 30, fill=C_POT, outline='', width=0)
        self.canvas.create_text(center_x, center_y - 10, text=
            f'POT: ${self.state.pot:.2f}', font=('Arial', 14, 'bold'), fill
            =C_TEXT)
        if self.state.to_call > 0:
            self.canvas.create_text(center_x, center_y + 10, text=
                f'To Call: ${self.state.to_call:.2f}', font=('Arial', 10),
                fill=C_TEXT_DIM)
        if self.state.equity is not None:
            self.canvas.create_text(center_x, center_y + 50, text=
                f'Equity: {self.state.equity:.1f}%', font=('Arial', 11,
                'bold'), fill='#10b981')

    @log_exceptions
    def update_state(self, active_players: Set[int], hero_seat: int,
        dealer_seat: int, pot: float, to_call: float, stage: str, equity:
        Optional[float]=None):
        """Update the table state and redraw."""
        logger.debug('Entering TableDiagramWindow.update_state',
            active_players=active_players, hero_seat=hero_seat, dealer_seat
            =dealer_seat)
...
if __name__ == '__main__':
    ...
    table.update_state(active_players={1, 2, 3, 4, 5, 6}, hero_seat=1,
        dealer_seat=3, pot=150.0, to_call=50.0, stage='Flop', equity=65.5)
    table.mainloop()
