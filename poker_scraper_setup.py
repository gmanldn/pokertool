...
class CardTemplateGenerator:
    ...
    def generate_from_screenshot(self, screenshot_path: str, regions: Dict):
...
class PokerSiteConfigurator:
    ...
    def interactive_setup(self):
        ...
        site_name = input("\nEnter poker site name (e.g., PokerStars, GGPoker): ").strip()
    ...
    def _get_table_size(self) -> Tuple[int, int]:
    ...
    def _configure_regions(self) -> Dict:
        ...
        # Essential regions
        essential_regions = [
            ('pot', 'Pot display area'),
            ('board', 'Community cards area'),
            ('hero_cards', 'Your hole cards area')
        ]

        for region_name, description in essential_regions:
            print(f"\n{description}:")
        ...
        for i in range(1, num_seats + 1):
            ...
            regions[f'seat_{i}'] = {'x': x, 'y': y, 'width': w, 'height': h}
    ...
    def _configure_colors(self) -> Dict:
        ...
        color_settings = [
            ('background', 'Table background', (35, 95, 63)),
            ('card_back', 'Card back', (150, 0, 0)),
            ('text', 'Text', (255, 255, 255)),
            ('button', 'Dealer button', (255, 215, 0)),
            ('chips', 'Chips', (100, 100, 255))
        ]

        for color_name, description, default in color_settings:
    ...
    def calibrate_from_screenshot(self, screenshot_path: str):
        ...
        print("Press 'q' to quit, 's' to save current configuration")

        regions = {}
        current_region = None

        def mouse_callback(event, x, y, flags, param):
            nonlocal current_region, regions

            if event == cv2.EVENT_LBUTTONDOWN:
                if current_region:
                    if current_region not in regions:
                        regions[current_region] = {'start': (x, y)}
                        print(f'  Start point for {current_region}: ({x}, {y})')
                    else:
        ...
        region_names = ['pot', 'board', 'hero_cards'] + [f'seat_{i}' for i in range(1, 10)]
...
def main():
    ...
    if choice == '1':
        ...
        generator.generate_from_screenshot(screenshot_path, regions)
